...
2. **A health path** that answers 200 only when the app can actually serve —
   not a route that returns 200 with the database down. It is `/api/health`
   here, declared as such in the platform's `apps.yml`; moving it means
   changing both. (`food` and `art` use `/health`; the path is per-app, not a
...
Migrations: Alembic. `entrypoint.sh` runs `alembic upgrade head` on every
container start, and the app ships `deploy/migrations` with `current`, `added`
and `downgrade` — the hook the platform's deploy pipeline calls to roll a
migration back, because reading a version table and reversing a migration are
specific to the tool an app uses and cannot be generic.
